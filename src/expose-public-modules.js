 ...
 */
function exposePublicModules(publicModules = [], compiler) {
...
exports.exposePublicModules = exposePublicModules;
/**
 * Generates a manifest describing modules and chunks.
 */
function getModuleLoaderManifest(modules, entryChunk, webpackOutputOptions, hash, publicModules) {
    ...
    let moduleObjs = modules.map(m => ({
        ...
        path: m.resource || '',
        relPath: m.resource && m.resource.substr(0, outputPath.length + 1) == outputPath + path_1.sep ? m.resource.substr(outputPath.length + 1) : m.resource || '',
        meta: m.meta
    }));
    // Filter all modules according to publicModules import.
    if (publicModules.length === 0)
        publicModules = [/(.*)/];
    moduleObjs = moduleObjs.filter(m => publicModules.find((matcher) => {
        if (m.request.match(/\.\.|\:|\\|\.\//))
            return false;
